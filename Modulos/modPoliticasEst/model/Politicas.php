...
class Politicas{
    public static function getPoliticasGeral(){
        ...
        case when dtfim < TO_DATE(sysdate) then
         ...
         FROM(
            SELECT  D.DTFIM, i.codprod, p.descricao, i.qt, d.qtdaplicacoesdesc QTMAX, d.qtminestparadesc QTMIN, e.qtestger-e.qtreserv-e.qtbloqueada estoque  from kokar.pcpedi i
                    inner join kokar.pcpedc c on c.numped = i.numped 
                    inner join kokar.pcregiao r on r.numregiao = c.numregiao  and r.numregiao = 1
                    ...
                    and p.descricao like '%ECONOMICO%'  and i.perdesc >= 49 and d.dtinicio < '10/10/2021' and d.codcli is null)
        ...
        case when dtfim < TO_DATE(sysdate) then
         ...
         FROM(
             SELECT  d.dtfim, i.codprod, p.descricao, i.qt, d.qtdaplicacoesdesc QTMAX, d.qtminestparadesc QTMIN, e.qtestger-e.qtreserv-e.qtbloqueada estoque  from kokar.pcpedi i
                     ...
                     and i.perdesc > 49 and d.dtinicio < '10/10/2021'  and d.codcli is null)
                     group by codprod, descricao, QTMAX, QTMIN, ESTOQUE,dtfim
                     order by descricao
        ");
        return $ret;
    ...
    public static function getPoliticasAnalitico(){
        ...
        $ret = $sql->select("SELECT numped, posicao, MES , data, codcli, codprod, descricao, sum(qt) qt, desconto, pvenda, qtmax, qtmin, estoque from (
            SELECT  i.numped, I.POSICAO,EXTRACT(MONTH FROM TO_DATE(C.DATA)) MES,c.data ||' '|| REPLACE(to_char(c.hora,'00')||':'||to_char(c.minuto,'00'), ' ','') data, i.codcli, i.codprod, p.descricao, i.qt, i.perdesc DESCONTO, i.pvenda, d.qtdaplicacoesdesc QTMAX, d.qtminestparadesc QTMIN, e.qtestger-e.qtreserv-e.qtbloqueada estoque  
            from kokar.pcpedi i
                    inner join kokar.pcpedc c on c.numped = i.numped 
                    inner join kokar.pcregiao r on r.numregiao = c.numregiao  and r.numregiao = 1
                    inner join kokar.pcprodut p on p.codprod = i.codprod
                    ...
                    AND I.perdesc >=30 and d.dtinicio > '10/10/2020' and d.dtfim < to_date(sysdate))
    ...
    public static function getPoliticasAnalitico2(){
      ...
      (SELECT D.DTFIM, d.codprod, p.descricao,  d.qtdaplicacoesdesc QTMAX,
              ...
              and d.percdesc > 49 and d.codcli is null and qtdaplicacoesdesc > 0
              ...
              left join(
                        ...
                        and i.perdesc > 49 and d.codcli is null
                        group by i.codprod)t2
                        on t1.codprod = t2.codprod
                        where dtfim > TO_DATE(sysdate)");
        return $ret;
    }
}