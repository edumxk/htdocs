...
class Carga{
    ...
    public static function getPendencias($numcarga){
        ...
        $arr = $sql->select("SELECT t.codprod, t.descricao, t.qt, t.peso,
            ...
            order by t.codprod",array(":numcarga"=>$numcarga)
        );
    ...
    public static function getSaldoCarga($cargas){
        ...
        $ret = $sql->select("SELECT codprod, descricao, qt, qtest
            from (
            SELECT i.codprod, p.descricao, sum(i.qt) qt, e.qtestger-e.qtreserv-e.qtbloqueada as qtest
            from cargac cc inner join cargai ci on cc.numcarga = ci.numcarga
            ...
            where cc.numcarga in ($varCargas)
              and c.posicao in ('P')
            group by i.codprod, p.descricao, e.qtestger-e.qtreserv-e.qtbloqueada
    ...
    public static function getDisponivel($carga){
        ...
        $ret = $sql->select("SELECT distinct numped
            from (
            select c.numped, i.codprod, i.qt, e.qtestger-e.qtbloqueada-e.qtreserv qtdisp,
                case when i.qt <= e.qtestger-e.qtbloqueada-e.qtreserv then 1
                    else 0 end as teste
            ...
            where t.teste = 1", array(":carga"=>$carga)
