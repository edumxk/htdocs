								...
								<tr onmouseover="listaHoverIn(this)" onmouseout="listaHoverOut(this)" >
									...
									<?php foreach ($faltas as $f) {
										if ($f->posicaof == 'P' && $f->clientef == $p->cod && $faux ==0){
											if ($p->consumidorfinal == 'S' && $p->uf != 'TO'){ ?>
												<td style="text-align: left; padding-left: 10px"> <i class="fa fa-male fa-2x"></i> <i class="fas fa-truck fa-lg"></i> <?php echo $p->cliente ?> </td>
													<?php $faux = 1; }else{  ?>
														<td style="text-align: left; padding-left: 10px"> <i class="fas fa-truck fa-lg"></i> <?php echo $p->cliente ?> </td>
													<?php $faux = 1; }
											}} if ($faux == 0) {
											if($p->consumidorfinal == 'S' && $p->uf != 'TO'){?>
											<td style="text-align: left; padding-left: 10px"> <i class="fa fa-male fa-2x"></i> <?php echo $p->cliente ?></td>
											<?php }else{ ?>
											<td style="text-align: left; padding-left: 10px"><?php echo $p->cliente ?> </td>
											<?php }}
											$faux = 0; ?>
