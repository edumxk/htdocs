...
<div class="container-fluid" style="background-color:lightgrey;">
	<div class="row" style="padding-top: 10px;">
		...
		<div class="col-md-1">
			<button style="height: 40px;" type="submit" class="btn btn-sm btn-primary" hidden onclick="openCadastro(1)">Cadastrar</button>	
		</div>
		<div class="col-md-1">
	...
	<div class="row">
		<div class="col-md-4" style="border: 1px solid black;">
			<h3 style="text-align: center;">
				<button type="button" onclick="openCadastro2(this)" class="btanque" value="2" style="width: 250px; height: auto">
					TINTAS 2000	
				</button>
			</h3>
						...
						<spawm style="padding-left:10px">
							<button type="button" onclick="editProducao(<?=$t->codproducao?>)" class="btanque" value="<?=$t->status?>" style="width: 200px; height:auto; text-align:justify">
								<table class="tbtanques2"> 
									<thead <?php if($t->status=='FINALIZADO'){} else{?> class="atraso" id="<?=$t->producao.':00'?>" <?php }?> onclick="editProducao(<?=$t->codproducao?>)">
		...
		<div class="col-md-4" style="border: 1px solid black;">
			<h3 style="text-align: center;">
				<button type="button" onclick="openCadastro2(this)" class="btanque" value="3" style="width: 250px; height: auto">
					TEXTURAS
				</button>
			</h3>
			<div class="row" style="padding-left:2px; padding-right:2px;">
								...
								<button type="button" onclick="editProducao(<?=$t->codproducao?>)" class="btanque" value="<?=$t->status?>" style="width: 200px; height: auto">
									<table class="tbtanques2"> 
										<thead <?php if($t->status=='FINALIZADO'){} else{?> class="atraso" id="<?=$t->producao.':00'?>" <?php }?> onclick="editProducao(<?=$t->codproducao?>)">
											<?php foreach($produtos as $p): 
												if($t->codproducao == $p->codproducao):?> 
												<th  onclick="editProducao(<?=$t->codproducao?>)"><?php echo($p->categoria.' '.$p->cor.'<br>T '.$t->codtanque.' | '.$p->peso.' KG | '.$t->status); if($t->status=='FINALIZADO'){$totpesof[$t->cod-1] += $p->peso;}else{$totpesop[$t->cod-1] += $p->peso;} ?> </th>
												<?php endif;  endforeach; ?>
										</thead>
											...
											<?php foreach ($item as $it):
												if($it->codproducao == $t->codproducao):?>
												<tbody>
													<td colspan="2"><?php echo $it->produto.' | '.$it->qt.' UN' ?></td>
													<?php if($t->status=='AGUARDANDO'):?>
														<td id="<?=$t->codproducao?>" onclick="excluir(this)" style="width: 15px; background-color:red"><b>x</b></td>
													<?php endif; ?>
												</tbody> 
		...
		<div class="col-md-4" style="border: 1px solid black;">
			<h3 style="text-align: center;">
				<button type="button" onclick="openCadastro2(this)" class="btanque" value="4" style="width: 250px; height: auto">
					MASSAS
				</button>
			</h3>
			<div class="row" style="padding-left:2px; padding-right:2px;">
							...
							<button type="button" onclick="editProducao(<?=$t->codproducao?>)" class="btanque" value="<?=$t->status?>" style="width: 200px; height: auto">
								<table class="tbtanques2"> 
									<thead <?php if($t->status=='FINALIZADO'){} else{?> class="atraso" id="<?=$t->producao.':00'?>" <?php }?>  onclick="editProducao(<?=$t->codproducao?>)">
										<?php foreach($produtos as $p): 
											if($t->codproducao == $p->codproducao):?> 
											<th colspan="1" onclick="editProducao(<?=$t->codproducao?>)"><?php echo($p->categoria.' '.$p->cor.'<br>T '.$t->codtanque.' | '.$p->peso.' KG | '.$t->status); if($t->status=='FINALIZADO'){$totpesof[$t->cod-1] += $p->peso;}else{$totpesop[$t->cod-1] += $p->peso;} ?> </th>
											<?php endif;  endforeach; ?>
									</thead>
	...
	<div class="row">
		<div class="col-md-2" style="border: 1px solid black;">
			<h3>
				<button type="button" onclick="openCadastro2(this)" class="btanque" value="1" style="width: 185px; height: auto">
					TINTAS 5000
				</button>
			</h3>
			<div class="row" style="padding-left:2px; padding-right:2px;">
		...
		<div class="col-md-10" style="border: 1px solid black;">
			<h3 style="text-align: center;">
				<button type="button" onclick="openCadastro2(this)" class="btanque" value="5"  id="5" style="width: 250px; height: auto">
					SOLVENTES
				</button>
			</h3>
	...
	<div class="row">
		<div class="col-md-12" style="padding-top:20px; padding-left: 70px; padding-right: 70px ">
							...
							<div class ="row">	
									...
									<div id="editar" class="editar">
										...
										<div class="row">
											...
											<div class="col-md-6" id="lapontamento" style="text-align: center">
										...
										<div class="row">
											...
											<div class="col-6 col-md-6" style="padding-bottom: 10px;" id="iapontamento">				
...
function openCadastro2(elm) {
	...
	function editar(elm) {	
			...
			status = $( "#sstatus option:selected" ).val();
			dataset = {"codproducao":codproducao, "codtanque":codtanque, "dtprevisao":dtprevisao, "hrprevisao":hrprevisao, "codfun":codfun,
				"produtos":{produtos}, "status": status, "dtfecha": dtfecha, "hrfecha": hrfecha} 
			console.log(dataset)
			$.ajax({
				type: 'POST',
				url: 'Controle/controlMapa.php',
	...
	function editProducao(elm) {
		...
		codproducao = elm;
			...
			$.ajax({
				type: 'POST',
				url: 'controle/controlMapa.php',
				data: { 'action': 'linhas2', codproducao},
				success: function (response) {
					console.log(response);
					$('#Linha').children().remove().end();
					$('#Linha').append(response);
...
#tabelaresultado .table-hover tbody tr:hover td {
  background: aqua;
}
