				...
				<table class="table-hover table-lg" id="tabelaresultado" style="border: 1px solid black;">
					...
					<tfoot>
						<tr style="font-size: large;width: auto;background-color:cyan">
							...
							<td> <b><?=number_format(array_sum($totpesof)+array_sum($totpesop)-($totpesof[4]+$totpesop[4]),0,',','.')?> KG </b></td>
							<th style="text-align: center;">TOTAL</th>
							<td> <b><?=number_format(($totpesof[4]+$totpesop[4]),0,',','.')?> KG </b></td>
						</tr>
						<tr>
						 	<th colspan="4" style="text-align: center; font-size:xx-large"> <b><?=number_format(array_sum($totpesop)+array_sum($totpesof),0,',','.')?> KG </b></th>
						</tr>
					</tfoot>
				</table>						
			</div>
