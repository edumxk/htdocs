					...
					<div class="row d-flex justify-content-center">
						<div class="col-md-12" style="font-size:large;">
							<div class ="row-md-12 row-12">								
									...
									<datalist id="tanque">
										<?php foreach($tanquesLista as $p):?>
											<option label="<?=$p['DESCRICAO']?>" value="<?=$p['CODTANQUE']?>"></option> 
										<?php endforeach;?>
									</datalist>
									<div class="tanque" id="divtanque">
										
										<input style="width:90%" id="tanque" list="tanque" placeholder="Selecione o Tanque">																												
...
function openCadastro(elm) {
	...
	function openCadastro2(elm) {
		...
		switch(elm.value){
			case '1':
			value1 ='<datalist id="tanque">'+
					<?php foreach($produtosList as $p):?>
						'<option label="<?=$p['DESCRICAO']?>" value="<?=$p['CODPROD']?>"></option>'+
					<?php endforeach;?>
					'</datalist>';
					$("#tanque").append(value1);
			break;
			case '2':
				valor = 5;
			break;
			...
			//console.log(value1)
			$('#modalCadastro').modal('toggle');
			$('#tanque').attr({'list':value1});
			//$("div.status").append(value1);
			$('#codproducao').val(0);
			$("div.editar select").val(valor).change();
					$('#horap').val("12:00")
					$('#datap').val("<?=$data?>")
