		...
		<div class="mapa_linhas">
			...
			?>
				<div class="mapa_linha_card <?= $l ?>">
					<h3>
						<button type="button" onclick="openCadastro2(this)" class="btanque" value="<?= $i ?>">
							<?= $l ?>
						</button>
					</h3>
					<div class="mapa_linhas_producao">
						...
						foreach ($tanques as $t) : ?>
							<?php if ($t->linha == $l) : ?>
								<span>
									<button type="button" onclick="editProducao(<?= $t->codproducao ?>)" class="btanque" value="<?= $t->status ?>">
										<table class="tbtanques2">
											<thead <?php if ($t->status == 'FINALIZADO') {
												...
												<?php foreach ($produtos as $p) :
													if ($t->codproducao == $p->codproducao) : ?>
														<th class="titulo-card" colspan="2" onclick="editProducao(<?= $t->codproducao ?>)">
															<?php echo ($p->categoria . ' ' . $p->cor . '<br>T ' . $t->codtanque . ' | ' . $p->peso . ' KG');
															if ($t->status == 'FINALIZADO') {
																$totpesof[$t->cod - 1] += $p->peso;
															} else {
																$totpesop[$t->cod - 1] += $p->peso;
															}
															?></th>
												<?php endif;
												endforeach; ?>
											...
											<tbody>
												<tr>
													<td class="status">Status: <?= $t->status ?></td>
												</tr>
												<tr>
													<td>
														<div class="contador" id="<?= $t->codproducao ?>"><input type="text" hidden disabled value="<?= $t->codproducao . "-" . $t->abertura ?>"></div>
													</td>
												</tr>
											</tbody>
										</table>
